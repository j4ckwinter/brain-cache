...
- [x] **STAT-05**: `brain-cache init` installs the status line script to `~/.brain-cache/statusline.mjs` and configures `~/.claude/settings.json` with the statusLine entry
- [x] **STAT-06**: `brain-cache init` reads existing `settings.json` before writing, merging the statusLine key without clobbering other user settings, and warns if a statusLine entry already exists
...
Requirements for Skill Reshape milestone. Each maps to roadmap phases.

### Core Reset

- [x] **SKILL-01**: Codebase contains only the v1.0 core services (embedder, chunker, crawler, lancedb, retriever, tokenCounter) plus incremental indexing — all v2.0+ feature services (flowTracer, cohesion, compression, fileWatcher, explainCodebase, configLoader) are removed
- [x] **SKILL-02**: MCP server exposes exactly 3 tools (index_repo, search_codebase, build_context) with no references to removed tools
...
| STAT-05 | Phase 32 | Complete |
| STAT-06 | Phase 32 | Complete |
| SKILL-01 | Phase 33 | Planned |
| SKILL-02 | Phase 33 | Planned |
| SKILL-03 | Phase 34 | Planned |
| SKILL-04 | Phase 34 | Planned |
| SKILL-05 | Phase 35 | Planned |
| SKILL-06 | Phase 35 | Planned |

**Coverage:**
- v2.4 requirements: 6 total, 6 complete
- v3.0 requirements: 6 total, 0 complete
- Mapped to phases: 12
- Unmapped: 0
