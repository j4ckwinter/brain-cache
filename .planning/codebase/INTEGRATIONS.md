...
**Connection:** HTTP to `http://localhost:11434` (default Ollama port)
**Auth:** None (local service)

**Service management (`src/services/ollama.ts`):**
- `isOllamaInstalled()` - Checks binary presence via `which`/`where`
- `isOllamaRunning()` - Pings `http://localhost:11434`
- `startOllama()` - Spawns `ollama serve` as detached process, polls for readiness (500ms intervals, 10 attempts)
- `pullModelIfMissing(model)` - Streams model download if not already present
- `getOllamaVersion()` - Runs `ollama --version`

**Embedding (`src/services/embedder.ts`):**
- `embedBatch(model, texts)` - Calls `ollama.embed({ model, input: texts })` with 120s timeout
- `embedBatchWithRetry(model, texts)` - Wraps embedBatch with single cold-start retry (5s delay) for ECONNRESET/ECONNREFUSED errors
...
| Model | Dimensions | VRAM Tier |
|-------|-----------|-----------|
| `nomic-embed-text` | 768 | `none` or `standard` (< 8 GiB) |
| `mxbai-embed-large` | 1024 | `large` (8+ GiB) |

Model selection is automatic based on detected VRAM via `src/services/capability.ts:selectEmbeddingModel()`.

### Anthropic Claude API

**Purpose:** Send assembled, token-budgeted context to Claude for reasoning (the `ask` command).
...
- `readIndexState(projectRoot)` / `writeIndexState(projectRoot, state)` - Manages `index_state.json` for tracking index metadata

**Search (`src/services/retriever.ts`):**
```typescript
table.query().nearestTo(queryVector).distanceType('cosine').limit(opts.limit).toArray()
```
- Distance threshold filtering: 0.3 cosine distance (0.7 similarity) for knowledge queries, 0.4 (0.6 similarity) for diagnostic queries
...
**Logging:**
- Pino ^9.0.0 with structured JSON output to stderr
...
|------|-------------|-------------|
| `index_repo` | Index a codebase (parse, chunk, embed, store) | `{ path: string }` |
...
**Pattern:** Each tool validates prerequisites (profile exists, Ollama running) before executing. Errors return `isError: true` with descriptive messages. Input schemas use Zod for validation.
...
- Extracts: functions, classes, methods (language-specific node types)
- Arrow functions filtered by AST depth (depth <= 5 = top-level/exported; depth > 5 = nested callback, skipped)
- Fallback: if no AST nodes found, emit single file-type chunk with full content
...
- Always excludes: `node_modules`, `.git`, `dist`, `build`, `.next`, `__pycache__`, lock files, minified JS
- Filters to supported source extensions only
...
- Apple Silicon: `system_profiler SPHardwareDataType -json` (checks `chip_type` contains "Apple M")
...
**Profile storage:** `~/.brain-cache/profile.json` (validated with `CapabilityProfileSchema`)
...
- `~/.brain-cache/config.json` - Global config (path defined but not yet used)
...
**Distribution:** npm package with `brain-cache` binary via `"bin"` field
