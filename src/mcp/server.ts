 ...
 * DEBT-03: Factory replaces module-level singleton; callable from tests without stdio.
 */
export function createMcpServer(): McpServer {
  const server = new McpServer({ name: "brain-cache", version });
  ...
  function buildContextResponse(result: ContextResult, query: string) {
    ...
    return {
      content: [{
        type: "text" as const,
        text: formatToolResponse(summary, `${formatContext(result)}\n\n${footer}`),
      }],
    };
  }
  ...
  server.registerTool(
    ...
    withGuards(
      ...
      { autoIndex: true, operationName: "Context build" },
    ),
  );

  // Tool 4: doctor (MCP-05)
  // doctor does not use withGuards — it reports health status regardless of profile/Ollama state (callers need diagnostics when guards would fail).
  server.registerTool(
    "doctor",
    {
      description:
    ...
    async ({ path: projectPath }) => {
      try {
        ...
        const ollamaVersion = installed ? await getOllamaVersion() : null;
